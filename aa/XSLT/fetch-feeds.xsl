...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:inv="http://wwww.investis.com/inv"
 xmlns:msxsl="urn:schemas-microsoft-com:xslt"  xmlns:js="urn:custom-javascript" exclude-result-prefixes="msxsl js inv ">
  ...
  <xsl:variable name="CurrYear" select="'2012'" />
        ...
        <xsl:choose>
          <xsl:when test="(position() >= 1 + ($CurrentPage - 1) * $ItemsPerPage) and (position() &lt; (1 + $CurrentPage * $ItemsPerPage))">
            <div>
              ...
              <!--RNS Date-->
              <p class="date">
                <xsl:value-of select="substring(./pubDate, 6,11)" /><span><xsl:value-of select="substring(./pubDate, 18,5)" /></span>
              ...
              <p class="story">
                <a target="_blank">
                  <xsl:attribute name="href">
                    <xsl:value-of select="concat($RNSDetailPageURL, js:makeUrl(string(./*[name()=$Link])))"/>
                  </xsl:attribute>
				 
                  <xsl:value-of select="./*[name()=$Title]"/>
                  <!--<xsl:choose>
                      <xsl:when test="$DateFormat='yyyy-MM-dd'">
                        <xsl:if test="contains(substring(./*[name()=$PubDate],1,4),$CurrYear)">
                          <xsl:attribute name="href">
                            <xsl:value-of select="concat($RNSDetailPageURL, js:makeUrl(string(./*[name()=$Link])))"/>
                          </xsl:attribute>
                          <xsl:value-of select="./*[name()=$Title]"/>
                      ...
                      <xsl:when test="$DateFormat='dd-MMM-yyyy'">
                        <xsl:if test="contains(substring(./*[name()=$PubDate],8,4),$CurrYear)">
                          <xsl:attribute name="href">
                            <xsl:value-of select="concat($RNSDetailPageURL, js:makeUrl(string(./*[name()=$Link])))"/>
                          </xsl:attribute>
                          <xsl:value-of select="./*[name()=$Title]"/>
                      ...
                      <xsl:when test="$DateFormat='Day,dd-MMM-yyyy'">
                        <xsl:if test="contains(substring(./*[name()=$PubDate],12,4),$CurrYear)">
                          <xsl:attribute name="href">
                            <xsl:value-of select="concat($RNSDetailPageURL, js:makeUrl(string(./*[name()=$Link])))"/>
                          </xsl:attribute>
                          <xsl:value-of select="./*[name()=$Title]"/>
